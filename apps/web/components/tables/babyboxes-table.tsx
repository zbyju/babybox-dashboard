...
export const columns: ColumnDef<Babybox>[] = [
  ...
  },
  {
    ...
    header: ({ column }) => {
      return (
        <div className="flex flex-row gap-1 items-center">
          ...
          <Button variant="ghost" size="icon" className="h-4 w-4" onClick={() => column.toggleSorting(column.getIsSorted() === "asc")}>
            <ArrowUpDown />
          </Button>
        </div>
      )
    ...
    cell: ({ getValue }) => (
      <span className="font-semibold text-[0.92rem]">{getValue() as string}</span>
    ),
    sortingFn: (rowA: Row<Babybox>, rowB: Row<Babybox>, columnId: string): number => {
    ...
    filterFn: (row: Row<Babybox>, columnId: string, filterValue: any): boolean => {
  ...
  },
  {
    ...
    cell: ({ row }) => {
      const timestamp = format(parse(row.getValue("lastData_timestamp"), "HH:mm:ss dd-MM-yyyy", new Date()), "HH:mm dd.MM.yyyy")
  ...
  },
  {
    ...
    cell: ({ row }) => {
      const d = parse(row.getValue("lastData_timestamp"), "HH:mm:ss dd-MM-yyyy", new Date())
      const now = new Date()
        ...
        return (
          <div className="text-center">
            <TooltipProvider delayDuration={300}>
              <Tooltip>
                <TooltipTrigger>
                  <Badge variant="default" className="bg-destructive text-destructive-foreground hover:bg-destructive mx-auto">Error</Badge>
                </TooltipTrigger>
                <TooltipContent>
                  <p>Data nepřišla - poslední záznam je více než 12 minut starý.</p>
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
          </div>
      ...
      return (
        <div className="text-center">
          <TooltipProvider delayDuration={300}>
            <Tooltip>
              <TooltipTrigger>
                <Badge variant="default" className="bg-success text-success-foreground hover:bg-success">OK</Badge>
              </TooltipTrigger>
              <TooltipContent>
