...
export default function BabyboxEditPageClient({
  slug,
}: BabyboxEditPageClientProps) {
  ...
  const [name, setName] = useState(data?.data?.name || slug);
  useEffect(() => {
  ...
  async function updateBabybox(babybox: BabyboxDetail) {
    try {
      const response = await fetch(
        `${babyboxServiceURL}/v1/babyboxes/${slug}`,
        {
          method: "PUT",
          headers: {
  ...
  return (
    <div className="mt-4 px-[5%] lg:px-[16%]">
      ...
      <div className="flex flex-row flex-wrap justify-between gap-4">
        <PageHeading heading="Editovat Informace" slug={slug} />
        <div className="flex flex-row flex-wrap gap-4">
          <Link href={`/dashboard/babybox/${slug}/detail`}>
